...
public class OperatorHyperlink extends Hyperlink implements ItemSelectionListener {
	...
	/** The operator text: is not between */
	public static final String OP_TEXT_REGEX = LocaleText.get("regex");
	...
	private int setupPopup(){
		...
		//not sure how this is any different from selecting "required"
		...
		if(dataType == QuestionDef.QTN_TYPE_TEXT ){
			//As best I can tell KoBo Collect doesn't support these, so they're gone.
			/*
			...
			count += 4;
			*/
			menuBar.addItem(OP_TEXT_REGEX,true,new SelectItemCommand(OP_TEXT_REGEX,this));	  
			count +=1;
		}
	...
	private int fromOperatorText2Value(String text){
		...
		else if(text.equals(OP_TEXT_REGEX))
			return ModelConstants.OPERATOR_REGEX;
	...
	public void setOperator(int operator){
		...
		else if(operator == ModelConstants.OPERATOR_REGEX)
			operatorText = OP_TEXT_REGEX;
