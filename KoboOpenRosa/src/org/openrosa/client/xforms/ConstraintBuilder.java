...
public class ConstraintBuilder {
	...
	public static void fromValidationRule2Xform(ValidationRule rule, FormDef formDef){
		QuestionDef questionDef = rule.getQuestion();
	 ...
	 */
	public static String fromValidationRuleCondition2Xform(Condition condition, FormDef formDef, int action, QuestionDef actionQtnDef){
		...
		if(questionDef != null)
		{
			...
			else if (condition.getOperator() == ModelConstants.OPERATOR_IN_LIST)
				 constraint = "selected(.,"+ value+")";
			else if (condition.getOperator() == ModelConstants.OPERATOR_NOT_IN_LIST)
				 constraint = "not(selected(.,"+ value+"))";
			else if (condition.getOperator() == ModelConstants.OPERATOR_REGEX)
