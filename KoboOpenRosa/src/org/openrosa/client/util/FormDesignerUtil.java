...
public class FormDesignerUtil {
	...
	private static String title = "KoBo FormDesigner";
	 ...
	 */
	public static void loadQuestions(List<IFormElement> questions, IFormElement refQuestion, MultiWordSuggestOracle oracle, boolean sameTypesOnly, QuestionDef parentQuestionDef){
		if(questions == null)
			return;
		...
		for(int i=0; i<questions.size(); i++){
			...
			//if it's a data element then skip it. We don't want to confuse ourselves with this stuff
			if(questionDef instanceof DataDefBase)
			{
				continue;
			}
			
			else if(refQuestion != null && refQuestion.getDataType() != questionDef.getDataType() && sameTypesOnly)
				continue;
			
			else if(questionDef == parentQuestionDef)
				continue;
			
			else if(questionDef == refQuestion)
				continue;
			...
			else if(questionDef instanceof GroupDef)
				loadQuestions(((GroupDef)questionDef).getChildren(),refQuestion,oracle,sameTypesOnly,parentQuestionDef);
				
			else if(questionDef.getDataType() == QuestionDef.QTN_TYPE_REPEAT)
				loadQuestions(((QuestionDef)questionDef).getRepeatQtnsDef().getQuestions(),refQuestion,oracle,sameTypesOnly,parentQuestionDef);
			
			else
				oracle.add(questionDef.getText() + "\t<"+questionDef.getBinding()+">");	
		}
	}
	...
	public static void loadQuestions(List<IFormElement> questions, QuestionDef refQuestion, MultiWordSuggestOracle oracle, boolean dynamicOptions){
		loadQuestions(questions, refQuestion, oracle, false,null);
	}
