...
public class SkipQtnsDialog  extends DialogBox {
	...
	public void setData(FormDef formDef, IFormElement questionDef, SkipRule skipRule){
		...
		if(skipRule != null && skipRule.getActionTarget() != null)
			loadSelQuestions(formDef, questionDef, skipRule.getActionTarget());
	}
	 ...
	 */
	private void loadSelQuestions(FormDef formDef, IFormElement questionDef, IFormElement selQuestion)
	{
		QuestionDef qtnDef = (QuestionDef)(selQuestion);
		
		if(qtnDef == null || qtnDef == questionDef)
			return;
		
		lbSelQtns.addItem(qtnDef.getText(), qtnDef.getBinding());
	}
	...
	{
		for(IFormElement qtnDef : elements)
		{
			if(qtnDef == questionDef)
				continue;
			
			if(skipRule != null && skipRule.getActionTarget() == qtnDef)
				continue;
			
			if(!(qtnDef instanceof QuestionDef) && !(qtnDef instanceof GroupDef) && !(qtnDef instanceof RepeatQtnsDef))
				continue;
			if(!(qtnDef instanceof GroupDef))
			{
				lbAllQtns.addItem(qtnDef.getText() + " - " + qtnDef.getPath(), qtnDef.getBinding());
			}
			
			//recurse, if it's not null.
			if(qtnDef.getChildren() != null)
			{
				loadPageQnts(qtnDef.getChildren(), questionDef, skipRule);
			}
		}
	}
