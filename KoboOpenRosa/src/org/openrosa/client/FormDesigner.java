...
public class FormDesigner implements EntryPoint, ResizeHandler {
	...
	public void onModuleLoad() {
		/*
		 * Applied in order to set the application theme for GXT
		 */
		...
		GWT.runAsync(new RunAsyncCallback() {
			...
			public void onSuccess() {
				...
				//DeferredCommand.addCommand(new Command() { //deprecated
	...
	public void onModuleLoadDeffered() {
		...
		try {
			RootPanel rootPanel = RootPanel.get("kobo_formdesigner");
			if (rootPanel == null) {
				FormUtil.dlg.hide();
				return;
			}
			...
			// Get rid of scrollbars, and clear out the window's built-in
			// margin,
			...
			// Finally, add the designer widget to the RootPanel, so that it
			// will be displayed.
			rootPanel.add(designer);

			// updateTabs();

			// If a form id has been specified in the html host page, load the
			// form
			// with that id in the designer.
			/*
			 * s = FormUtil.getFormId(); if(s != null)
			...
			scheduler.scheduleDeferred(new Command() {
				public void execute() {
					designer.onWindowResized(Window.getClientWidth(), Window
							.getClientHeight());
	...
	/*
	 * private void updateTabs(){ String s =
	 * FormUtil.getDivValue("showXformsSourceTab"); if(!("1".equals(s) ||
	 * "true".equals(s))) designer.removeXformSourceTab();
	 * 
	 * s = FormUtil.getDivValue("showLayoutXmlTab"); if(!("1".equals(s) ||
	 * "true".equals(s))) designer.removeLayoutXmlTab();
	 * 
	 * s = FormUtil.getDivValue("showLanguageTab"); if(!("1".equals(s) ||
	 * "true".equals(s))) designer.removeLanguageTab();
	 * 
	 * s = FormUtil.getDivValue("showModelXmlTab"); if(!("1".equals(s) ||
	 * "true".equals(s))) designer.removeModelXmlTab();
	 * 
	 * s = FormUtil.getDivValue("showJavaScriptTab"); if(!("1".equals(s) ||
	 * "true".equals(s))) designer.removeJavaScriptTab(); }
	 */
	...
	private native void publishJS() /*-{
		...
		$wnd.closeSaveToFileDialog = @org.openrosa.client.view.SaveToFileDialog::closeInstance();
		$wnd.closeOpenFromFileDialog = @org.openrosa.client.view.OpenFromFileDialog::closeInstance();
		$wnd.settingsCloseDialog = @org.openrosa.client.view.KoboSettingsDialog::closeInstance();
		$wnd.closeSaveBlockDialog = @org.openrosa.client.view.SaveToLibraryDialog::closeInstance();
		$wnd.importDataFromLibrary = @org.openrosa.client.view.AddBlockFromLibraryDialog::addFromLibrary();
		$wnd.importDataFromLibraryCloseDialog = @org.openrosa.client.view.AddBlockFromLibraryDialog::closeInstance();
	}-*/;
	...
	private void loadLocales() {
		...
		for (String token : tokens) {
			...
			locales.add(new Locale(token.substring(0, index).trim(), token
