...
import org.openrosa.client.jr.core.model.DataBinding;
import org.openrosa.client.jr.core.model.FormDef;
import org.openrosa.client.jr.core.model.GroupDef;
import org.openrosa.client.jr.core.model.IDataReference;
import org.openrosa.client.jr.core.model.IFormElement;
import org.openrosa.client.jr.core.model.ItemsetBinding;
...
import org.openrosa.client.jr.core.model.condition.Recalculate;
import org.openrosa.client.jr.core.model.condition.Triggerable;
...
import org.openrosa.client.jr.xforms.util.XFormAnswerDataParser;
import org.openrosa.client.jr.xforms.util.XFormSerializer;
...
public class XFormParser {
	...
	public static FormDef getFormDef(Document doc){
		...
		defaultNamespace = doc.getDocumentElement().getNamespaceURI(); //getNamespaceUri(null); ???????????????
		
		parseElement(formDef, doc.getDocumentElement(), formDef, topLevelHandlers);
...
//			boolean allowUnknownElements, boolean allowText, boolean recurseUnknown) {
		...
		if (eh != null) {
			...
			for (int i = 0; i < e.getChildNodes().getLength() /*getChildCount()*/; i++) { //???????????
				Node child = e.getChildNodes().item(i);
				if (child.getNodeType() == Node.ELEMENT_NODE) {
					parseElement(f, (Element)child, parent, handlers);
				}
	...
	protected static QuestionDef parseControl (IFormElement parent, Element e, FormDef f, int controlType) {
		...
		String bind = e.getAttribute("bind");
	...
	private static String getLabel (Element e, FormDef f){
		if(e.getChildNodes().getLength() == 0) return null;
		...
		for(int i = 0; i<e.getChildNodes().getLength();i++){
			...
			if(kid.getNodeType()!=Node.TEXT_NODE /*&& !(e.getChild(i) instanceof String)*/){
				...
				if(NAMESPACE_HTML.equals(child.getNamespaceURI())) {
					...
					//Otherwise, ignore it.
					System.out.println("Unrecognized tag inside of text: <"  + child.getNodeName() + ">. " +
...
/*	private static String getLabel (Element e, FormDef f) {
		...
		for (int i = 0; i < e.getChildCount(); i++) {
			if (e.getType(i) != Node.TEXT) {
				if (e.getType(i) == Node.ELEMENT && "output".equals(e.getElement(i).getNodeName())) {
					outputFound = true;
	...
	*/
	...
	private static String parseOutput (Element e, FormDef f) {
		...
		String xpath = e.getAttribute("ref");
		if (xpath == null) {
	...
	private static void parseItemset (FormDef f, QuestionDef q, Element e, IFormElement qparent) {
		...
		String nodesetStr = e.getAttribute("nodeset");
		if(nodesetStr == null ) throw new RuntimeException("No nodeset attribute in element: ["+e.getNodeName()+"]. This is required. (Element Printout:"+XFormSerializer.elementToString(e)+")");
		...
		for (int i = 0; i < e.getChildNodes().getLength(); i++) {
			...
			if ("label".equals(childName)) {
				String labelXpath = child.getAttribute("ref");
				...
				String copyRef = child.getAttribute("ref");
				...
				String valueXpath = child.getAttribute("ref");
	...
	private static void parseGroup (IFormElement parent, Element e, FormDef f, int groupType) {
		...
		String ref = e.getAttribute("ref");
		String nodeset = e.getAttribute("nodeset");
		String bind = e.getAttribute("bind");
		group.setAppearanceAttr(e.getAttribute("appearance"));
		...
		if (group.getRepeat()) {
			...
			String countRef = e.getAttribute("count");
			if (countRef != null) {
				group.count = getAbsRef(new XPathReference(countRef), parent);
				group.noAddRemove = true;
			} else {
				group.noAddRemove = (e.getAttribute("noAddRemove") != null);				
			}
		}
	...
	private static TreeReference getFormElementRef (IFormElement fe) {
		if (fe instanceof FormDef) {
			...
			ref.add(instanceNode.getNodeName(), 0);
			return ref;
		} else {
...
//						repeat.setShortTextID(group.getShortTextID(), null);						
	...
	private static void parseTextHandle (TableLocaleSource l, Element text, FormDef f) {
		...
		for (int k = 0; k < text.getChildNodes().getLength(); k++) {
			...
			String form = value.getAttribute("form");
	...
	private static void parseBind (FormDef f, Element e) {
		...
		binding.setId(e.getAttribute("id"));
		...
		binding.setDataType(getDataType(e.getAttribute("type")));
		...
		String xpathConstr = e.getAttribute("constraint");
		if (xpathConstr != null) {
			try {
			...
			binding.constraintMessage = e.getAttribute("constraintMsg");
		}
		...
		binding.setPreload(e.getAttribute("preload"));
		binding.setPreloadParams(e.getAttribute("preloadParams"));
	...
	private static void parseInstance (FormDef f, Element e) {
		...
		String schema = e.getNamespaceURI(); //getNamespace(); ???????????
	...
	public static TreeElement buildInstanceStructure (Element node, TreeElement parent) {
		...
		if (hasElements && hasText) {
			System.out.println("Warning: instance node '" + node.getNodeName() + "' contains both elements and text as children; text ignored");
		}
		
		//check for repeat templating
		String name = node.getNodeName();
		int multiplicity;
		if (node.getAttribute("template") != null) {
		...
		String modelType = node.getAttribute("modeltype");
	...
	private static void verifyBindings (FormDef f, FormInstance instance) {
		...
		for (int i = 0; i < refs.size(); i++) {
			...
			if (ref.size() <= 1) {
				throw new XFormParseException("Cannot bind repeat to '/' or '/" + instanceNode.getNodeName() + "'");
			}
		}
		...
		if (hasElements) {
			...
			for (int i = 0; i < numChildren; i++) {
				...
				if (kid.getNodeType() == Node.ELEMENT_NODE) {
					...
					int index;
					boolean isTemplate = (child.getAttribute("template") != null);
	...
	/*public static FormInstance parseDataModelGhettoooooo (InputStream instanceXMLStream, InputStream formDefXMLStream, String locale) {
		...
		parseElement(formDef, formDefXML.getDocumentElement(), formDef, topLevelHandlers);
		collapseRepeatGroups(formDef);
		
		instanceNode = instanceXML.getDocumentElement(); //replace default form instance with our new instance
		parseInstance(formDef, instanceNode);
	...
	public static String getXMLText (Node n, boolean trim) {
		return (n.getChildNodes().getLength() == 0 ? null : getXMLText(n, 0, trim));
	}
	...
	**/
	...
	public static String getVagueElementPrintout(Element e, int maxDepth) {
		String elementString = "<" + e.getNodeName();
		for(int i = 0; i <  e.getAttributes().getLength(); ++i) { //getAttributeCount()  ?????????
			elementString += " " + e.getAttributes().item(i).getNodeName() + "=\"";
			elementString += e.getAttributes().item(i).getNodeValue() + "\"";
		}
