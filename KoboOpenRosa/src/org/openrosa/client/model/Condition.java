...
public class Condition implements Serializable{
	...
	private static final long serialVersionUID = 2526615224970648383L;

	/** The unique identifier of the question referenced by this condition. */
	private QuestionDef questionId = null;
	...
	public Condition(Condition condition){
		this(condition.getId(),condition.getQuestion(),condition.getOperator(),condition.getFunction(),condition.getValue());
	}
	 ...
	 */
	public Condition(int id,QuestionDef questionId, int operator, int function, String value) {
		this();
	...
	/**
	 * returns true if the operator is only numeric, like less than greater than, otherwise returns false
	 * @return
	 */
	public boolean isOperatorNumeric()
	{
		if(operator == ModelConstants.OPERATOR_GREATER ||
			operator == ModelConstants.OPERATOR_GREATER_EQUAL || 
			operator == ModelConstants.OPERATOR_LESS ||
	...
	public void setQuestion(QuestionDef questionId) {
	...
	public boolean isTrue(FormDef formDef, boolean validation){
		...
		try{
			QuestionDef qn = this.questionId;
			if(qn == null)
				return false; //possibly question deleted

			if(value.startsWith(formDef.getVariableName()+"/")){
				QuestionDef qn2 = (QuestionDef) (formDef.getChild(value.substring(value.indexOf('/')+1)));
				if(qn2 != null){
					value = qn2.getAnswer();
	...
	public String getValue(FormDef formDef){	
		if(value.startsWith(formDef.getVariableName()+"/")){
			QuestionDef qn = (QuestionDef) (formDef.getChild(value.substring(value.indexOf('/')+1)));
			if(qn != null)
				return qn.getAnswer();
		}
		return value;
	...
	public boolean hasQuestion(QuestionDef questionDef, FormDef formDef){
		if(value.startsWith(formDef.getVariableName()+"/")){
			QuestionDef qtn = (QuestionDef) (formDef.getChild(value.substring(value.indexOf('/')+1)));
	...
	public QuestionDef getQuestion(FormDef formDef){
		if(value.startsWith(formDef.getVariableName()+"/"))
			return (QuestionDef) (formDef.getChild(value.substring(value.indexOf('/')+1)));
	...
	{
		String retVal = this.questionId.toString() + " ";
		retVal += ModelConstants.operatorStringMap[this.operator]+" " + this.value;
		return retVal;
		
	}
	
	//checks to see if two conditions effectively say the same thing
	public boolean effectivelyEqualTo(Condition c)
	{
		if(getOperator() != c.getOperator())
		{
			return false;
		}
		else if (getFunction() != c.getFunction())
		{
			return false;
		}
		else if (!getValue().equals(c.getValue()))
		{
			return false;
		}
		else if (!getSecondValue().equals(c.getSecondValue()))
		{
			return false;
		}
		else if(getQuestion() != c.getQuestion())
		{
			return false;
		}
