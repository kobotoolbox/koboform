...
public class ValidationRule implements Serializable{
	 ...
	 * 
	 */
	private static final long serialVersionUID = -8450403131906934441L;

	/** The unique identifier of the question referenced by this validation rule. */
	private QuestionDef referencedQuestionDef = null;
	...
	public ValidationRule(QuestionDef questionDef, FormDef formDef){
		this.referencedQuestionDef = questionDef;
		this.formDef = formDef;
	}
	
	/** Copy constructor. */
	public ValidationRule(ValidationRule validationRule){
		setQuestion(validationRule.getQuestion());
	 ...
	 * @param actionTargets
	 */
	public ValidationRule(QuestionDef questionId, Vector<Condition> conditions , String errorMessage) {
	...
	public QuestionDef getQuestion() {
		return referencedQuestionDef;
	}

	public void setQuestion(QuestionDef questionId) {
		this.referencedQuestionDef = questionId;
	}
	...
	public void removeQuestion(IFormElement questionDef)
	{
		for(int index = 0; index < getConditionCount(); index++)
		{
			...
			if(condition.getQuestion() == questionDef || 
					(condition.getValueQtnDef() != null && condition.getValueQtnDef() == questionDef)){
	...
	public void refresh(FormDef dstFormDef, FormDef srcFormDef){
		...
		QuestionDef qtn = referencedQuestionDef;
		if(qtn == null)
		...
		ValidationRule validationRule = new ValidationRule(questionDef,dstFormDef);
		validationRule.setConditionsOperator(getConditionsOperator());
		validationRule.setErrorMessage(getErrorMessage());
		
		for(int index = 0; index < getConditionCount(); index++){
			Condition condition = getConditionAt(index);
			qtn = condition.getQuestion();
			if(qtn == null)
				continue;
			
			questionDef = (QuestionDef) (dstFormDef.getChild(qtn.getBinding()));
			if(questionDef == null)
				continue;
